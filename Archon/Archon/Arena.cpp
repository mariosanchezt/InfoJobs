...
void Arena::iniciarCombate(Pieza* p1, Pieza* p2) {
    ...
    // LUZ siempre a la izquierda, OSCURIDAD siempre a la derecha
    ...
    cLuz->pieza = piezaLuz;
    cLuz->pos = sf::Vector2f(OFFSET_X + 80.f, OFFSET_Y + ALTO / 2.f);
...
void Arena::generarObstaculos() {
    ...
    int numObs = 4 + rand() % 4;
    int intentos = 0;
    while ((int)obstaculos.size() < numObs && intentos < 100) {
        intentos++;
        Obstaculo obs;
        obs.pos.x = OFFSET_X + 180.f + (float)(rand() % (int)(ANCHO - 360.f));
        obs.pos.y = OFFSET_Y + 80.f + (float)(rand() % (int)(ALTO - 160.f));
        obs.tam = 25.f + (float)(rand() % 20);

        float d1 = std::sqrt(std::pow(obs.pos.x - combatiente1.pos.x, 2) + std::pow(obs.pos.y - combatiente1.pos.y, 2));
        float d2 = std::sqrt(std::pow(obs.pos.x - combatiente2.pos.x, 2) + std::pow(obs.pos.y - combatiente2.pos.y, 2));
        if (d1 > 100.f && d2 > 100.f)
...
bool Arena::colisionaConObstaculo(sf::Vector2f pos, float radio) {
    for (const auto& obs : obstaculos) {
        float dist = std::sqrt(std::pow(pos.x - obs.pos.x, 2) + std::pow(pos.y - obs.pos.y, 2));
...
bool Arena::dentroDeArena(sf::Vector2f pos) {
    return pos.x >= OFFSET_X + TAM_PIEZA &&
        pos.x <= OFFSET_X + ANCHO - TAM_PIEZA &&
        pos.y >= OFFSET_Y + TAM_PIEZA &&
        pos.y <= OFFSET_Y + ALTO - TAM_PIEZA;
}
...
void Arena::update(float dt) {
    ...
    if (spaceAhora && !luz.teclaDsparoPulsada) crearProyectil(luz, oscuridad);
    luz.teclaDsparoPulsada = spaceAhora;

    // OSCURIDAD: Flechas + Enter
    ...
    if (enterAhora && !oscuridad.teclaDsparoPulsada) crearProyectil(oscuridad, luz);
