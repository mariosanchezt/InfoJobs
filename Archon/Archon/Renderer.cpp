...
void Renderer::dibujarAnimaciones() {
    for (const auto& anim : animaciones) {
        ...
        case ANIM_CONGELAR:   colorFlash = sf::Color(120, 200, 255, alpha); break;
        default: continue;
        }
...
void Renderer::dibujarEstadoTablero(
    ...
    if (piezaCongelada != nullptr) dibujarBordeEfecto(piezaCongelada->filaInicial, piezaCongelada->colInicial, sf::Color(120, 200, 255), t);
    if (piezaRalentizada != nullptr) dibujarBordeEfecto(piezaRalentizada->filaInicial, piezaRalentizada->colInicial, sf::Color(220, 180, 30), t);
    if (piezaFortalecida != nullptr) dibujarBordeEfecto(piezaFortalecida->filaInicial, piezaFortalecida->colInicial, sf::Color(255, 200, 0), t);
...
void Renderer::dibujarPieza(Pieza* p, int fila, int col) {
    ...
    if (tieneSprite) {
        ...
        sprite.setPosition(sf::Vector2f(px + (TAM_CASILLA - spriteW) / 2.f, py + (TAM_CASILLA - spriteH) / 2.f));
        ventana.draw(sprite);
    }
    else {
...
void Renderer::dibujarCursor(Tablero* tablero) {
    ...
    if (modoHechizo == HECHIZO_ALIADO)  colorCursor = sf::Color(80, 120, 255, 220);
    else if (modoHechizo == HECHIZO_ENEMIGO) colorCursor = sf::Color(255, 100, 30, 220);
    else if (modoHechizo == HECHIZO_CASILLA) colorCursor = sf::Color(200, 80, 255, 220);
...
void Renderer::dibujarEstadoArena(const Arena& arena) {
    ...
    fondo.setFillColor(sf::Color(40, 35, 55));
    ...
    for (const auto& obs : arena.getObstaculos()) {
        ...
        c.setFillColor(sf::Color(100, 80, 60));
        c.setOutlineColor(sf::Color(60, 50, 40));
    ...
    for (const auto& p : arena.getProyectiles()) {
        ...
        sf::Color colProyectil = p.esDeLuz ? sf::Color(100, 255, 120) : sf::Color(255, 80, 80);
        sf::CircleShape circulo(Arena::getTamProyectil() / 2.f);
        circulo.setFillColor(colProyectil);
        circulo.setPosition(sf::Vector2f(p.pos.x - Arena::getTamProyectil() / 2.f, p.pos.y - Arena::getTamProyectil() / 2.f));
        ventana.draw(circulo);
    }

    dibujarCombatienteArena(arena.getCombatiente1(), arena.getCombatiente1().pieza->getBando() == LUZ);
    dibujarCombatienteArena(arena.getCombatiente2(), arena.getCombatiente2().pieza->getBando() == LUZ);
...
void Renderer::dibujarCombatienteArena(const CombatienteArena& c, bool esLuz) {
    ...
    dibujarBarraVida(c.pieza->vida, c.pieza->vidaMaxima, c.pos.x - anchoVida / 2.f, c.pos.y - radio - 14.f, anchoVida);
}
...
void Renderer::dibujarPanelHechizos(Bando turno, bool* hechizosUsadosLuz, bool* hechizosUsadosOscuridad, int hechizoSeleccionado) {
    ...
    for (int i = 0; i < 7; i++) {
        ...
        if (listaUsados[i])       fondo.setFillColor(sf::Color(60, 60, 60));
        else if (i == hechizoSeleccionado) fondo.setFillColor(sf::Color(215, 215, 0, 180));
        else    fondo.setFillColor(turno == LUZ ? sf::Color(30, 80, 40) : sf::Color(80, 30, 30));
