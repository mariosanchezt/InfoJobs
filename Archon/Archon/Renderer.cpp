    ...
    : ventana(vent)
{
    ...
    // Colores del tablero: blanco/negro tipo ajedrez + gris pa las casillas oscilantes
    colorBlanco = sf::Color(240, 217, 181); // crema
    colorNegro = sf::Color(90, 120, 80); // verde oscuro
    colorGris = sf::Color(160, 160, 140); // gris neutro
...
void Renderer::dibujarEstadoTablero(Tablero* tablero) {
    if (tablero == nullptr) return;

    dibujarTablero(tablero);

    // Resaltado encima de todo lo demas
...
void Renderer::dibujarTablero(Tablero* tablero) {
    for (int f = 0; f < 9; f++) {
        for (int c = 0; c < 9; c++) {
            ...
            // Color de la casilla segun el ciclo del tablero
            ...
            // Dibujamos la pieza si hay una en esa casilla
...
void Renderer::dibujarPieza(Pieza* p, int fila, int col) {
    ...
    // Color del circulo segun bando
    sf::Color colorPieza = (p->getBando() == LUZ)
        ? sf::Color(80, 180, 100) // verde plantas
        : sf::Color(180, 60, 60); // rojo zombies

    // Circulo que representa la pieza
    ...
    // Centramos el circulo en la casilla
    circulo.setPosition(sf::Vector2f(px + TAM_CASILLA / 2.f - radio, py + TAM_CASILLA / 2.f - radio - 8.f));
    ventana.draw(circulo);

    // Nombre de la pieza (solo si hay fuente cargada)
    if (fuenteCargada) {
        // Nombre abreviado pa q quepa en la casilla (primeras 4 letras)
        ...
        // Centramos el texto debajo del circulo
        ...
        // Barra de vida mini encima del circulo
...
void Renderer::dibujarBarraVida(float vida, float vidaMax, float x, float y, float ancho) {
    ...
    // Relleno segun vida restante
    sf::Color colorVida;
    if (porcentaje > 0.6f) colorVida = sf::Color(50, 200, 50); // verde
    else if (porcentaje > 0.3f) colorVida = sf::Color(230, 180, 30); // amarillo
    else                        colorVida = sf::Color(220, 50, 50); // rojo
...
void Renderer::dibujarEstadoArena(Pieza* p1, Pieza* p2) {
    ...
    // Fondo oscuro pa distinguirlo del tablero
...
void Renderer::seleccionarCasilla(int fila, int col) {
