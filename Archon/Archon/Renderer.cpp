...
void Renderer::cargarSheetSiNecesario(const std::string& clave, const std::string& carpeta) {
    // Funcion mantenida para compatibilidad con Renderer.h.
    // En esta version los sprites principales se cargan como texturas normales.
}
...
void Renderer::cargarSprites(const std::string& carpeta) {
    std::vector<std::string> archivos = {
        ...
        "pea.png",
        "Cabbage_proyectil.png",
        "Cactus_proyectil.png",
        "Starfruit_proyectil.png",
        "Balloon_proyectil.png",
        "Catapult_proyectil.png",
        "Digger_proyectil.png",
        "Football_proyectil.png",
        "Cob_Cannon_proyectil.png",
        "Dr. Zomboss_proyectil.png"
    };
    ...
    for (const auto& archivo : archivos) {
        ...
        if (img.loadFromFile(ruta)) {
            if (archivo == "Starfruit.png" || archivo == "Chomper.png") {
    ...
    for (const auto& par : proyectiles) {
        const std::string& nombre = par.first;
        const std::string& archivo = par.second;

        if (texturas.count(archivo) > 0) {
            texturasProyectil[nombre] = texturas[archivo];
        }
    }
}
...
void Renderer::dibujarAnimaciones() {
    for (const auto& anim : animaciones) {
        ...
        sf::CircleShape interior(radio2);
        interior.setFillColor(sf::Color(
            colorFlash.r,
            colorFlash.g,
            colorFlash.b,
            alpha / 3
        ));
        interior.setOrigin(sf::Vector2f(radio2, radio2));
        interior.setPosition(sf::Vector2f(cx, cy));
        ventana.draw(interior);
    }
}

void Renderer::dibujarTeleportEnCurso() {
    if (animTeleport.fase == FASE_NINGUNA || animTeleport.pieza == nullptr) {
        return;
    }
    ...
    if (animTeleport.fase == FASE_HUNDIR) {
        ...
        sf::Vector2f centro = getCentroCasilla(
            animTeleport.filaOrigen,
            animTeleport.colOrigen
        );

        centro.y += TAM_CASILLA * 0.3f * p;

        dibujarPiezaPixelEscala(
            animTeleport.pieza,
            centro.x,
            centro.y,
            escala
        ...
        sf::Vector2f centro = getCentroCasilla(
            animTeleport.filaDest,
            animTeleport.colDest
        );
...
void Renderer::dibujarEstadoTablero(
    ...
    dibujarTiempoGeneral(tiempoGeneral);

    dibujarPanelPoderBando(
        tablero,
        LUZ,
        CEMENTERIO_X_LUZ,
        PODER_PANEL_Y,
...
void Renderer::dibujarTablero(Tablero* tablero) {
    for (int f = 0; f < 9; f++) {
        for (int c = 0; c < 9; c++) {
            ...
            if (colorActual == 0) {
                colorFondo = colorBlanco;
            }
            else if (colorActual == 1) {
                colorFondo = colorNegro;
            }
            else {
            ...
            casilla.setPosition(sf::Vector2f(
                OFFSET_X + c * TAM_CASILLA,
                OFFSET_Y + f * TAM_CASILLA
            ));
...
void Renderer::dibujarMovimientosDisponibles() {
    for (const auto& pos : movimientosDisponibles) {
        int f = pos.first;
        int c = pos.second;
...
void Renderer::dibujarBarraVida(
    float vida,
    float vidaMax,
    float x,
    float y,
    ...
    if (porcentaje > 0.6f) {
        colorVida = sf::Color(50, 200, 50);
    }
    else if (porcentaje > 0.3f) {
        colorVida = sf::Color(230, 180, 30);
    }
    else {
        colorVida = sf::Color(220, 50, 50);
    }
...
void Renderer::dibujarBarraCooldown(
    float tiempoRecarga,
...
void Renderer::dibujarIndicadorTurno(
    Bando turno,
    ...
    if (modoHechizo == HECHIZO_ALIADO) {
        colorFranja = sf::Color(30, 80, 160);
    }

    if (modoHechizo == HECHIZO_ENEMIGO) {
        colorFranja = sf::Color(160, 60, 20);
    }

    if (modoHechizo == HECHIZO_CASILLA) {
        colorFranja = sf::Color(100, 30, 130);
    }
    ...
    if (modoHechizo == HECHIZO_ALIADO) {
        ...
        if (animTeleport.fase != FASE_NINGUNA) {
            ...
            textoInferior = nombrePiezaTeleport +
                " seleccionado  |  Muevelo a la casilla de destino  |  Esc: cancelar";
        }
        else {
        ...
        if (tablero != nullptr) {
            ...
            if (bajoCursor != nullptr && bajoCursor->getBando() == turno) {
                textoInferior = bajoCursor->getNombre() +
                    "  |  Space/Enter: seleccionar  |  H: hechizos  |  I: sugerencia";
            }
                ...
                textoInferior = esLuz
                    ? "TURNO: PLANTAS  |  H: abrir hechizos  |  I: sugerencia"
                    : "TURNO: ZOMBIES  |  H: abrir hechizos  |  I: sugerencia";
            }
        }
        else {
...
void Renderer::dibujarCursor(Tablero* tablero) {
    ...
    if (modoHechizo == HECHIZO_ALIADO) {
        ...
        colorCursor = (p != nullptr && p->getBando() == LUZ)
            ? sf::Color(40, 255, 40, 220)
            : sf::Color(255, 60, 60, 220);
    }
    else {
...
void Renderer::dibujarCementerio(
    ...
    for (const auto& par : cementerio) {
        if (indice >= maxItems) {
            ...
            mas.setPosition(sf::Vector2f(
                x + CEMENTERIO_ANCHO / 2.f - 10.f,
                y + CEMENTERIO_ALTO - 36.f
            ));
...
void Renderer::dibujarTiempoGeneral(float tiempoGeneral) {
    if (!fuenteCargada) {
        return;
    }
    ...
    panel.setPosition(sf::Vector2f(x, y));
    panel.setFillColor(sf::Color(15, 15, 25, 220));
    panel.setOutlineColor(sf::Color(180, 180, 230, 180));
    panel.setOutlineThickness(2.f);

    ventana.draw(panel);

    std::string textoTiempo = "TIEMPO GENERAL: " + formatearTiempo(tiempoGeneral);

    sf::Text texto(fuente, textoTiempo, 18);
    texto.setFillColor(tiempoGeneral <= 60.f
        ? sf::Color(255, 90, 90)
...
void Renderer::dibujarPanelPoderBando(
    ...
    if (!fuenteCargada || tablero == nullptr) {
        return;
    }
    ...
    sf::Text conteo(fuente, textoConteo, 19);
    conteo.setFillColor(sf::Color::White);
    conteo.setStyle(sf::Text::Bold);

    sf::FloatRect bc = conteo.getLocalBounds();

    conteo.setPosition(sf::Vector2f(
        x + CEMENTERIO_ANCHO / 2.f - bc.size.x / 2.f,
        y + 36.f
    ));

    ventana.draw(conteo);

    dibujarCirculosPoder(puntos, x + 21.f, y + 70.f, colorTitulo);

    sf::RectangleShape separador(sf::Vector2f(CEMENTERIO_ANCHO - 24.f, 2.f));
    separador.setPosition(sf::Vector2f(x + 12.f, y + 103.f));
    separador.setFillColor(sf::Color(colorTitulo.r, colorTitulo.g, colorTitulo.b, 150));

    ventana.draw(separador);

    std::string tituloPuntuacion = (bando == LUZ)
        ? "PUNTUACION PLANTAS"
        : "PUNTUACION ZOMBIES";

    sf::Text txtPuntuacionTitulo(fuente, tituloPuntuacion, 13);
    txtPuntuacionTitulo.setFillColor(colorTitulo);
    txtPuntuacionTitulo.setStyle(sf::Text::Bold);

    sf::FloatRect bpt = txtPuntuacionTitulo.getLocalBounds();

    txtPuntuacionTitulo.setPosition(sf::Vector2f(
        x + CEMENTERIO_ANCHO / 2.f - bpt.size.x / 2.f,
        y + 116.f
    ));

    ventana.draw(txtPuntuacionTitulo);

    sf::Text txtPuntuacion(fuente, std::to_string(puntuacion), 25);
    txtPuntuacion.setFillColor(sf::Color::White);
    txtPuntuacion.setStyle(sf::Text::Bold);

    sf::FloatRect bp = txtPuntuacion.getLocalBounds();

    txtPuntuacion.setPosition(sf::Vector2f(
        x + CEMENTERIO_ANCHO / 2.f - bp.size.x / 2.f,
        y + 136.f
    ));

    ventana.draw(txtPuntuacion);

    sf::Text txtTiempoTitulo(fuente, "TIEMPO", 13);
    txtTiempoTitulo.setFillColor(colorTitulo);
    txtTiempoTitulo.setStyle(sf::Text::Bold);

    sf::FloatRect btt = txtTiempoTitulo.getLocalBounds();

    txtTiempoTitulo.setPosition(sf::Vector2f(
        x + CEMENTERIO_ANCHO / 2.f - btt.size.x / 2.f,
        y + 168.f
    ));

    ventana.draw(txtTiempoTitulo);

    sf::Color colorTiempo = tiempoBando <= 30.f
        ? sf::Color(255, 80, 80)
        : sf::Color(230, 230, 240);

    sf::Text txtTiempo(fuente, formatearTiempo(tiempoBando), 23);
    txtTiempo.setFillColor(colorTiempo);
    txtTiempo.setStyle(sf::Text::Bold);

    sf::FloatRect bti = txtTiempo.getLocalBounds();

    txtTiempo.setPosition(sf::Vector2f(
        x + CEMENTERIO_ANCHO / 2.f - bti.size.x / 2.f,
        y + 185.f
    ));

    ventana.draw(txtTiempo);
}

void Renderer::dibujarCirculosPoder(
...
void Renderer::dibujarEstadoArena(const Arena& arena) {
    ...
    if (tienePatio) {
        ...
        fondo.setScale(sf::Vector2f(
            Arena::getAncho() / (float)texSize.x,
            Arena::getAlto() / (float)texSize.y
        ));
    ...
    for (const auto& obs : arena.getObstaculos()) {
        if (tieneNuez) {
            ...
            c.setPosition(sf::Vector2f(
                obs.pos.x - obs.tam / 2.f,
                obs.pos.y - obs.tam / 2.f
            ));
    ...
    for (const auto& p : arena.getProyectiles()) {
        ...
        circulo.setOrigin(sf::Vector2f(
            Arena::getTamProyectil() / 2.f,
            Arena::getTamProyectil() / 2.f
        ));
...
void Renderer::dibujarPuntosDePoder(Tablero* tablero) {
    ...
    for (const auto& punto : puntos) {
        ...
        if (p != nullptr) {
            if (p->getBando() == LUZ) {
                colorDiamante = sf::Color(80u, 220u, 120u, 220u);
                colorBorde = sf::Color(40u, 160u, 80u, 230u);
            }
            else {
                colorDiamante = sf::Color(220u, 80u, 80u, 220u);
                colorBorde = sf::Color(160u, 40u, 40u, 230u);
            }
        }
...
void Renderer::dibujarPantallaVictoria(
    ...
    std::string fondoVictoria = esLuz
        ? "Plantas_Victoria.png"
        : "Zombies_Victoria.png";
    ...
    auto textoC = [&](const std::string& txt, int tam, sf::Color col, float y) {
        ...
        t2.setPosition(sf::Vector2f(
            VENTANA_ANCHO / 2.f - b.size.x / 2.f,
            y
        ));
    ...
    tv.setFillColor(colorGanador);
    tv.setStyle(sf::Text::Bold);

    sf::FloatRect b1 = tv.getLocalBounds();

    tv.setPosition(sf::Vector2f(
        VENTANA_ANCHO / 2.f - b1.size.x / 2.f,
        panelY + 25.f
    ));
    ...
    textoC(
        esLuz ? "El jardin ha sido defendido" : "Los zombies han tomado el jardin",
        24,
        sf::Color(230u, 230u, 230u),
        panelY + 103.f
    );
    ...
    textoC(txtTipo, 23, sf::Color::White, panelY + 150.f);

    textoC(
        "Turnos jugados: " + std::to_string(turnosJugados),
        19,
        sf::Color(220u, 220u, 220u),
        panelY + 192.f
    );

    textoC(
        "Puntuacion plantas: " + std::to_string(puntuacionLuz),
        19,
        sf::Color(120u, 240u, 150u),
        panelY + 220.f
    );

    textoC(
        "Puntuacion zombies: " + std::to_string(puntuacionOscuridad),
        19,
        sf::Color(240u, 120u, 120u),
        panelY + 246.f
    );

    textoC(
        "Tiempo general restante: " + formatearTiempo(tiempoGeneral),
        18,
        sf::Color(220u, 220u, 240u),
        panelY + 274.f
    );

    textoC(
        "Tiempo plantas: " + formatearTiempo(tiempoLuz) +
        "   |   Tiempo zombies: " + formatearTiempo(tiempoOscuridad),
        17,
        sf::Color(220u, 220u, 220u),
        panelY + 300.f
    );
    ...
    textoC(
        "Piezas eliminadas: " + std::to_string(bajasTotales),
        18,
        sf::Color(220u, 220u, 220u),
        panelY + 328.f
    );
    ...
    textoC(
        "Bajas plantas: " + std::to_string(bajasLuz) +
        "   |   Bajas zombies: " + std::to_string(bajasOscuridad),
        17,
        sf::Color(220u, 220u, 220u),
        panelY + 354.f
    );
    ...
    textoC(
        "Pulsa ENTER o haz click para volver al menu",
        17,
        sf::Color(190u, 190u, 190u),
        panelY + 392.f
    );
}
void Renderer::dibujarPanelHechizos(
    ...
    for (int i = 0; i < 7; i++) {
        ...
        texto.setFillColor(listaUsados[i]
...
void Renderer::dibujarSugerencia(int fOrigen, int cOrigen, int fDestino, int cDestino) {
    ...
    origen.setPosition(sf::Vector2f(
        OFFSET_X + cOrigen * TAM_CASILLA + 2.f,
        OFFSET_Y + fOrigen * TAM_CASILLA + 2.f
    ));
    ...
    destino.setPosition(sf::Vector2f(
        OFFSET_X + cDestino * TAM_CASILLA + 2.f,
        OFFSET_Y + fDestino * TAM_CASILLA + 2.f
    ));
